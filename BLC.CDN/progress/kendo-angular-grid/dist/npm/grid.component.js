...
var GridComponent = (function () {
    function GridComponent(supportService, selectionService, wrapper, groupInfoService, groupsService, changeNotification, detailsService, editService, filterService, pdfService, responsiveService, renderer, excelService, ngZone, rtl) {
        var _this = this;
    ...
    Object.defineProperty(GridComponent.prototype, "showGroupFooters", {
        get: function () {
            return this.columnList.filter(function (column) { return column.groupFooterTemplateRef; }).length > 0;
        },
        enumerable: true,
        configurable: true
    });
    ...
    GridComponent.prototype.verifySettings = function () {
        if (core_1.isDevMode()) {
            ...
            if (this.lockedLeafColumns.length && this.columnList.filter(function (x) { return !x.width; }).length) {
            ...
            if (this.columnList.filter(function (x) { return column_group_component_1.isColumnGroupComponent(x); }).filter(function (x) { return x.parent && !column_group_component_1.isColumnGroupComponent(x.parent); }).length) {
                throw new Error('ColumnGroupComponent can be nested only inside another ColumnGroupComponent');
            }
            if (this.columnList.filter(function (x) { return column_group_component_1.isColumnGroupComponent(x); }).filter(function (x) { return x.children.length < 2; }).length) {
