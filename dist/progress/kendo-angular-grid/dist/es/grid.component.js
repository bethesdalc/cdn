...
var GridComponent = (function () {
    ...
    GridComponent.prototype.verifySettings = function () {
        if (isDevMode()) {
            ...
            if (this.columnList.filter(function (x) { return isColumnGroupComponent(x); }).filter(function (x) { return x.parent && !isColumnGroupComponent(x.parent); }).length) {
                throw new Error('ColumnGroupComponent can be nested only inside another ColumnGroupComponent');
            }
            if (this.columnList.filter(function (x) { return isColumnGroupComponent(x); }).filter(function (x) { return x.children.length < 2; }).length) {
